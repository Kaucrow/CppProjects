...
def copy_header(DATA_FOLDER, dest_document, teacher_name, teacher_ci, teacher_type):
    ...
    if teacher_type == 'TUTOR':
        header_document = Document(DATA_FOLDER + '/in/format/header_tutor.docx')
    elif teacher_type == 'JURADO':
        header_document = Document(DATA_FOLDER + '/in/format/header_jury.docx')
    else:
        raise Exception(f'Error: No valid teacher_type case for value {teacher_type}')
